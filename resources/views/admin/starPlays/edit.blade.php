...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.starPlay.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.star-plays.update", [$starPlay->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $starPlay->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('star') ? 'is-invalid' : '' }}" name="star_id" id="star_id" required>
                    @foreach($stars as $id => $entry)
                        <option value="{{ $id }}" {{ (old('star_id') ? old('star_id') : $starPlay->star->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('star'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('payed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="payed" id="payed" value="1" {{ $starPlay->payed || old('payed', 0) === 1 ? 'checked' : '' }}>
