...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.star.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.stars.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('limit') ? 'is-invalid' : '' }}" type="number" name="limit" id="limit" value="{{ old('limit', '0') }}" step="1" required>
                @if($errors->has('limit'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('benefectors') ? 'is-invalid' : '' }}" name="benefectors[]" id="benefectors" multiple required>
                    @foreach($benefectors as $id => $benefector)
                        <option value="{{ $id }}" {{ in_array($id, old('benefectors', [])) ? 'selected' : '' }}>{{ $benefector }}</option>
                    @endforeach
                </select>
